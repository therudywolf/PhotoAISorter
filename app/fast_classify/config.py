...
class FastClassifySettings:
    ...
    softmax_temperature: float = 0.065
    vlm_fallback: bool = True
    exemplar_boost: float = 1.32
    text_prompt_fusion: float = 0.68
    device: str = "auto"
    use_fp16: bool = True
    cache_embeddings: bool = True
    multi_crop: bool = True
    multi_crop_views: int = 5
    ...
    def from_dict(
        ...
        *,
        ...
        s = cls(
            quality=str(raw.get("quality", QUALITY_MAX) or QUALITY_MAX),
            model_name=str(raw.get("model_name", "ViT-L-14") or "ViT-L-14"),
            pretrained=str(raw.get("pretrained", "openai") or "openai"),
            batch_size=max(1, min(512, int(raw.get("batch_size", 36)))),
            image_max_side=max(128, min(768, int(raw.get("image_max_side", 512)))),
            confidence_threshold=max(
                0.05, min(0.95, float(raw.get("confidence_threshold", 0.22)))
            ),
            min_margin=max(0.01, min(0.5, float(raw.get("min_margin", 0.10)))),
            softmax_temperature=max(
                0.01, min(0.2, float(raw.get("softmax_temperature", 0.065)))
            ),
            vlm_fallback=bool(raw.get("vlm_fallback", True)),
            exemplar_boost=max(1.0, min(2.5, float(raw.get("exemplar_boost", 1.32)))),
            text_prompt_fusion=max(
                0.0, min(1.0, float(raw.get("text_prompt_fusion", 0.68)))
            ),
            ...
            multi_crop_views=max(1, min(5, int(raw.get("multi_crop_views", 5)))),
            prefetch_workers=max(1, min(16, int(raw.get("prefetch_workers", 8)))),
            video_frames=max(1, min(12, int(raw.get("video_frames", 7)))),
